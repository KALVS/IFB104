...
####This function is used to make the GUI
def gui_maker():
    # Make GUI
    gui_window = Tk()
    ...
    ## extra button to save invoice in Database.
    global databaser
    databaser = Button(gui_window, text ="Would you like to save your invoice?",
                            command = data_base)
    ...
    databaser.grid(row=9,column=0, columnspan=10)
    step_three.grid(row=10, column=0, columnspan=10)
    progress.grid(row=11, column=0, columnspan=10)
    file_name = 'file:///Users/alexholm/Documents/IFB104/N9918205.Assignment2/invoice.html'

    gui_window.mainloop()

### When print invoice is clicked, it will generate
### A HTML document with the following content
def print_invoice():
    data_base()
...
"""
    ...
    """
    ####creating the table of items for the invoice.
    #for every integer in the spinbox, collect 1 title, 1 image and 1 price.
    #multiple price by 1.33
...
###again
    ...
    item_count = 0
#no need for *1.33 as it is Australian
...
#Function for making 10 empty labels so I can grid my GUI
def make_grid():
    column_number = 0
